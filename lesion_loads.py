...
def get_lesion_neo_loads(lesion_to_effectvariants, variant_to_neos):
    result = dict('total': 0, 'fs': 0, 'nonsyn': 'inframe_indel': 'fs_trunc': 0, 'pre_stop': 0)
...
def check_annotation(variant, snpeff_ann, varcode_ann, outfile):
    ...
    if set([annotation_mapping[effect] for effect in varcode_effects]) != set(snpeff_effects):
        f.write(f'{variant}\t{(', ').join(list(set(snpeff_effects)))}\t{(', ').join(list(set(varcode_effects)))}')
    elif len(list(set(snpeff_effects))) > 1 or len(list(set(varcode_effects))) > 1:
        f.write(f'{variant}\t{(', ').join(list(set(snpeff_effects)))}\t{(', ').join(list(set(varcode_effects)))}')
...
def get_lesion_variants(lesions, patients, args):
    ...
    for lesion, patient in zip(lesions, patients):
        lesion_to_effectvariants[lesion] = dict('total': (0,[]), 'fs':(0,[]), 'nonsyn':(0,[]), 'inframe_indel':(0,[]), 'fs_trunc':(0,[]), 'pre_stop':(0,[]))
