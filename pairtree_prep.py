...
def main():
    ...
    with open(ssm_file, 'w') as ssm:
        ...
        for lines in zip(*file_handles):
            ...
            var_reads = [str(int(total) - int(ref)) for total,ref in zip(total_reads, ref_reads)] if not args.total_alt else ref_reads
        ...
        for vcf, sample in zip(vcf_files, sample_names):
            ...
            with open(vcf, 'r') as infile, open(ssm_file, 'w') as outfile:
                ...
                for line in infile.readlines():
                    ...
                    var_reads = int(total_reads) - int(ref_reads) if not args.total_alt else ref_reads
